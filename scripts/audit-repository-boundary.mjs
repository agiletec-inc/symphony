...
const legacyAuthorities = ['deployment/symphony/upstream.json', 'deployment/symphony/repo-lanes.json', 'deployment/symphony/linear-github-issues-sync.json']
...
export async function auditRepositoryBoundary({ manifestPath, repositoryName, checkout }) {
  ...
  const findings = [], policyPath = resolve(checkout, lane.workflowPath)
  ...
  if (lane.workflowPath !== 'WORKFLOW.md' && await exists(legacyWorkflow)) {
    ...
    for (const value of [instance.tracker.projectSlug, instance.tracker.commonRequiredLabel, lane.requiredLabel]) {
      if (source.includes(value)) findings.push(`legacy WORKFLOW.md duplicates instance authority: ${value}`)
    }
  }
  for (const relative of legacyAuthorities) if (await exists(resolve(checkout, relative))) findings.push(`legacy consumer-owned instance authority remains: ${relative}`)
  if (findings.length > 0) throw new Error(`[symphony-boundary] repository migration incomplete:\n- ${findings.join('\n- ')}`)
  return { lane, policyPath }
}

if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const [manifestPath, repositoryName, checkout] = process.argv.slice(2)
  if (!manifestPath || !repositoryName || !checkout) throw new Error('usage: audit-repository-boundary.mjs <instance.json> <lane> <repo-checkout>')
