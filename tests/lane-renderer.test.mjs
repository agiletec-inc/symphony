...
test('combines instance and repository policy with remote-first branch guards', async () => { const options = await fixture(); const workflow = await renderLaneWorkflow({ ...options, repositoryName: 'agiletec' }); assert.match(workflow, /airis-symphony-pilot/); assert.match(workflow, /repo:agiletec/); assert.match(workflow, /Repository policy/); assert.match(workflow, /git fetch --atomic --prune origin/); assert.match(workflow, /git switch --detach/); assert.match(workflow, /git switch --create "\$branch"/); assert.match(workflow, /refusing to run on the default branch/) })
